...
public:
	...
	void operator() (void)
	{
		...
		if (Property::getImage (txd, Drawer::value (), 0) == false)				{return;}
...
//		PropertySet		*ds, *us;
							   ...
							   const UI::DrawStyle& drawStyle)
	{
		...
		glColor4fv (drawStyle.frame ());			drawSparkline (plotHistogramShader, minValues, maxValues, hdata, hmaxs, datRegion, attributeID);

		// draw the regions
		glLineWidth (2.0f);
		glColor4f (1.0f, 1.0f, 1.0f, 1.0f);			UI::Bounds::drawShortEdges (GL_LINES, datRegion);
#if 0
		...
		// label the ranges
		glColor4fv (drawStyle.text ());				Mutables::DRW_Range::drawRange (datRegion, min.str (), max.str ());
	}
#endif
public:
	DRW_Attribute ()
...
#else
		if ((repositorySet && value ()) == false)				{setReturnValue (FALSE); return;}

		Mutables::PropertySet&	clrm = repositorySet[value ()];

		if ((clrm && "Image\0") == false)						{setReturnValue (FALSE); return;}

		if (clrm["Image\0"].getImage (txd) == false)			{setReturnValue (FALSE); return;}
#endif
